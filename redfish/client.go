...
func setupClientWithConfig(ctx context.Context, config *ClientConfig) (c *APIClient, err error) {
	...
	client.Service, err = redfishapi.ServiceRoot(client)
	if err != nil {
		return nil, err
	}
...
func (c *APIClient) CloneWithSession() (*APIClient, error) {
	...
	service, err := redfishapi.ServiceRoot(&newClient)
	if err != nil {
		return nil, err
	}
